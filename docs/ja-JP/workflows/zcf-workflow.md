...
**例**：
```
/workflow ユーザーログイン機能を実装、メールと電話番号ログインをサポート
```
...
> 💡 **ヒント**：Codex は `/prompts:` を使用し、Claude Code は `/skill-name`（例：`/workflow`）で skills を起動します。
...
1. **skill を起動**：`/workflow` または（Codex）`/prompts:workflow` を入力してタスクを説明
...
```
# メインタスク
/workflow ユーザー管理システムを構築

# サブタスク 1
/workflow ユーザー登録機能を実装

# サブタスク 2
/workflow ユーザーログイン機能を実装

# サブタスク 3
/workflow ユーザー情報管理を実装
```
...
# 2. 6段階ワークフローで詳細を実装
/workflow コメントの CRUD 操作と権限制御を実装
...
```
/workflow マイクロサービスアーキテクチャのユーザー認証システムを構築、OAuth2 と JWT をサポート
```
...
```
/workflow ユーザーモジュールをリファクタリング、コード品質と保守性を向上
```
